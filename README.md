...
**Флаги `import_osm_stops`:**

| Флаг | Назначение | По умолчанию |
|------|-----------|--------------|
| `--city ГОРОД` | Город для поиска остановок (по name в OSM) | Оренбург |
| `--bbox S,W,N,E` | Прямоугольник координат вместо поиска по городу | — |
| `--neighbours N` | Сколько ближайших соседей соединять в графе | 4 |
| `--merge-radius N` | Расстояние (м) для объединения дубликатов (разные стороны улицы) | 50 |
| `--dry-run` | Только запросить OSM и показать статистику, без записи в БД | — |
| `--no-graph` | Не строить граф связности | — |
| `--clear` | Удалить все остановки/связи перед импортом | — |

**Примеры:**

```bash
# Предпросмотр для Москвы (без записи в БД)
docker compose exec backend python -m app.seeds.import_osm_stops --city Москва --dry-run

# Импорт Москвы с мягкой дедупликацией (радиус 30 м)
docker compose exec backend python -m app.seeds.import_osm_stops --city Москва --merge-radius 30

# Импорт по точному прямоугольнику
docker compose exec backend python -m app.seeds.import_osm_stops --bbox 51.7,55.0,51.9,55.3
```

> **Примечание:** Overpass API — публичный сервис, при большой нагрузке может
> вернуть таймаут. В этом случае используйте `--bbox` с узким прямоугольником
...
| GET   | `/api/v1/routes/{id}`              | Карточка маршрута со списком остановок и геометрией |
| POST  | `/api/v1/routes/build`             | Расчёт маршрута с промежуточными точками (OSRM/Dijkstra/A*) |
